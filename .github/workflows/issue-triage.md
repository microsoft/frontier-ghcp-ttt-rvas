...
This repository is a GitHub Copilot and Agentic Workflows train-the-trainer curriculum and MkDocs site. Issues may describe curriculum gaps, trainer-guide updates, lab problems, slide rendering failures, MkDocs build issues, GitHub Actions automation, or requests for new learning tracks.

Be careful with the `squad` and `squad:*` labels: this repository already has separate Squad workflows that use those labels for agent assignment. Do not apply `squad` labels unless the issue explicitly asks for Squad routing or the repository label conventions clearly support it.
...
- Prefer curriculum/content labels such as `documentation`, `type:docs`, `type:feature`, `type:bug`, or `go:needs-research` when they exist and match the issue.
