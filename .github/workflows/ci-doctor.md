...
This repository is a GitHub Copilot and Agentic Workflows train-the-trainer curriculum built as a MkDocs Material site. The monitored workflow renders Marp slide decks, renders trainer guides, builds the MkDocs site, and deploys to GitHub Pages.

When diagnosing failures, prioritize these local reproduction commands:

- Install site dependencies: `pip install -r requirements.txt`
- Render trainer guides: `python scripts/render-trainer-guides.py`
- Render slides: `npm install -g @marp-team/marp-cli && npx puppeteer browsers install chrome && bash scripts/build-slides.sh`
- Build the site: `mkdocs build --strict`

Common failure areas include broken MkDocs navigation or links, missing generated slide/trainer artifacts, Marp rendering issues in `sessions/*/slides.md`, and Markdown/content changes that violate the repository's linting conventions.
