...
Scan the repository for merged pull requests and content changes from the last 24 hours, identify curriculum, delivery, automation, or site changes that should be documented, and update the documentation accordingly.
...
- **Curriculum Changes**: New sessions, labs, trainer guidance, slides, tracks, prerequisites, or delivery instructions
...
- The MkDocs site uses the repository root as `docs_dir`.
- Root documentation includes `README.md` and `curriculum-plan.md`.
- Track documentation lives in `tracks/`.
- Session documentation lives in `sessions/session-*/README.md`, `sessions/session-*/lab/README.md`, `sessions/session-*/slides.md`, and `sessions/session-*/trainer-content/README.md`.
...
Do not edit generated `site/` or `output/` files. If navigation changes are needed, update `mkdocs.yml` with the corresponding content changes.
...
- **Validation**: When documentation changes affect the MkDocs site or navigation, validate with `pip install -r requirements.txt && mkdocs build --strict`. When trainer guides or slides change, also validate the corresponding renderer (`python scripts/render-trainer-guides.py` or `bash scripts/build-slides.sh`).
